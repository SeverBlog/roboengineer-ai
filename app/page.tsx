...
export default function Home(){
 ...
 const vote=async(item:string)=>{if(submitting||!ROADMAP.includes(item))return;setSubmitting("vote:"+item);setRoadmapVote(item);localStorage.setItem("re_vote",item);record("roadmap_vote");try{const supabase=getSupabase();if(!supabase)throw new Error("Supabase unavailable");const{error}=await supabase.from("feature_votes").insert({feature_name:item});if(error)throw error;showNotice("投票已提交，感谢你的选择")}catch{showNotice("投票已保存到本机，在线提交暂时失败")}finally{setSubmitting("")}};
 const submitFeedback=async(e:FormEvent)=>{e.preventDefault();if(submitting)return;const content=feedback.trim(),contact=email.trim();if(content.length<2||content.length>2000){showNotice("反馈内容需要 2–2000 个字符");return}if(contact.length>254){showNotice("邮箱长度不能超过 254 个字符");return}setSubmitting("feedback");const list=JSON.parse(localStorage.getItem("re_feedback")||"[]");list.unshift({feedback:content,email:contact,createdAt:new Date().toISOString()});localStorage.setItem("re_feedback",JSON.stringify(list.slice(0,30)));record("feedback_submitted");try{const supabase=getSupabase();if(!supabase)throw new Error("Supabase unavailable");const{error}=await supabase.from("feedback").insert({content,email:contact||null});if(error)throw error;setFeedback("");setEmail("");showNotice("反馈已提交，感谢你的建议")}catch{showNotice("反馈已保存到本机，在线提交暂时失败")}finally{setSubmitting("")}};
 const pricingInterest=async(plan:string)=>{if(submitting)return;record("pricing_"+plan);if(plan==="free"){showNotice("已记录你的方案偏好");return}const planName=plan==="pro"?"Pro":plan==="team"?"Team":"";if(!planName)return;setSubmitting("pricing:"+plan);try{const supabase=getSupabase();if(!supabase)throw new Error("Supabase unavailable");const{error}=await supabase.from("pricing_interests").insert({plan_name:planName,email:email.trim()||null});if(error)throw error;showNotice(`已记录你对 ${planName} 方案的兴趣`)}catch{showNotice("偏好已记录到本机，在线提交暂时失败")}finally{setSubmitting("")}};
 ...
 return <main>
  ...
  <section className="growth shell" id="pricing">
   ...
   <div className="pricing-grid"><article><span>Free</span><h3>免费版</h3><strong>¥0 <small>/ 永久</small></strong><p>力矩计算、基础电机筛选、基础 BOM、方案本机保存。</p><button className="ghost" disabled={Boolean(submitting)} onClick={()=>pricingInterest("free")}>继续免费使用</button></article><article className="featured"><em>需求验证中</em><span>Pro</span><h3>专业版</h3><strong>¥49 <small>/ 月</small></strong><p>完整电机数据库、无限项目、PDF 报告、URDF 与高级计算。</p><button className="primary" disabled={Boolean(submitting)} onClick={()=>pricingInterest("pro")}>{submitting==="pricing:pro"?"正在提交…":"我对 Pro 感兴趣"}</button></article><article><span>Team</span><h3>企业版</h3><strong>¥499 <small>/ 月起</small></strong><p>私有器件库、团队项目、内部知识库和工程 API。</p><button className="ghost" disabled={Boolean(submitting)} onClick={()=>pricingInterest("team")}>{submitting==="pricing:team"?"正在提交…":"记录企业需求"}</button></article></div>
   <div className="validation-grid"><div className="roadmap"><span className="kicker">下一步做什么</span><h3>请投票选择最需要的功能</h3>{ROADMAP.map(item=><button key={item} disabled={Boolean(submitting)} className={roadmapVote===item?"selected":""} onClick={()=>vote(item)}><span>{roadmapVote===item?"✓":"○"}</span>{item}<i>{submitting==="vote:"+item?"提交中":"投票"}</i></button>)}</div><form className="feedback" onSubmit={submitFeedback}><span className="kicker">用户反馈</span><h3>你现在最难解决的问题是什么？</h3><textarea required minLength={2} maxLength={2000} value={feedback} onChange={e=>setFeedback(e.target.value)} placeholder="例如：算出力矩后，不知道如何选择减速比和电机……"/><input type="email" maxLength={254} value={email} onChange={e=>setEmail(e.target.value)} placeholder="邮箱（选填，仅用于后续联系）"/><button className="primary" disabled={Boolean(submitting)} type="submit">{submitting==="feedback"?"正在提交…":"提交反馈 →"}</button><small>反馈会安全提交用于产品改进，并在本机保留一份。</small></form></div>
  </section>
  ...
  <footer className="shell footer"><a className="brand" href="#top"><span>R</span>RoboEngineer <b>AI</b></a><p>机器人研发，从一个真正有用的小工具开始。</p><small>工程参数本地计算 · 反馈与意向经授权提交</small></footer>
 </main>
}
